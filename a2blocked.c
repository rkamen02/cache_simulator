...
{
	int target_block_bytes = 1 * pow( 2, 10);
	int blocksize;
	
	/* 
	 * So that more than one cell is stored in a block, 
	 * if size is not zero and less than 64 kB, calculate how many may be
	 * stored in a block of target_block_bytes bytes.
	 */
	if ((size != 0) && (size < (64 * pow( 2, 10)))) {
		/* find how many cells may fit in the block -> how many per edge? */
		blocksize = floor( sqrt( target_block_bytes / size));
